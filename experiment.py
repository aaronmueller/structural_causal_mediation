...
class Intervention():
    ...
    def __init__(self,
        ...
        # need some special hacking for this case; change 1st verb
        if structure == "across_subj_rel":
            tokens = self.base_strings[1].split()
            if tokens[3].endswith("s"):
                tokens[3] = tokens[3][:-1]
            else:
                tokens[3] = tokens[3] + "s"
            self.base_strings[1] = " ".join(tokens)
...
class StrTemplates():
    def __init__(self,
        ...
        # All the initial strings
        # First item should be neutral, others tainted
        self.base_strings = {'sing': [], 'pl': []}
        # singular-subject templates
        if structure == "simple_agreement":
            ...
            # plural-subject templates
            for N2 in grammar[('N2', frozenset('s'))]:
                for V_tr in grammar[('V_tr', frozenset('s'))]:
                    self.base_strings['pl'].append(base_string.format('{}', N2, V_tr))
        elif structure == "across_subj_rel":
            for N2 in grammar[('N2', frozenset('p'))]:
                for V_tr in grammar[('V_tr', frozenset('s'))]:
                    self.base_strings['sing'].append(base_string.format('{}', V_tr, N2))
            for N2 in grammar[('N2', frozenset('s'))]:
                for V_tr in grammar[('V_tr', frozenset('p'))]:
            ...
            for N1 in grammar[('N1', frozenset('s'))]:
                self.base_strings['pl'].append(base_string.format(N1, '{}'))
...
class Model():
    ...
    def neuron_intervention_single_experiment(self,
        ...
        with torch.no_grad():
            ...
            diffnum_representations = self.get_representations(
            ...
            # e.g. The teacher said that
                ...
                replace_or_diff = 'replace'
            '''
            # START OF STRUCTURAL CASES
            if intervention_type == 'diffnum_direct':
                context = intervention.base_strings_tok[1]
                rep = base_representations
                replace_or_diff = 'replace'
            elif intervention_type == 'diffnum_indirect':
                context = intervention.base_strings_tok[0]
                rep = diffnum_representations
                replace_or_diff = 'replace'
            else:
                raise ValueError(f"Invalid intervention_type: {intervention_type}")
