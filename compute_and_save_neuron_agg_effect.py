...
def get_all_effects(fname, direction="man"):
    """
...
def main(folder_name="results/20191114_neuron_intervention/", model_name="distilgpt2"):
    ...
    diffnum_files = [
        f
        for f in paths
        if "diffnum_indirect" in f
        if os.path.exists(f.replace("indirect", "direct"))
    ]
    diffnum_dfs = []
    for path in diffnum_files:
        diffnum_dfs.append(get_all_effects(path))
    diffnum_df = pd.concat(diffnum_dfs)
    ...
    diffnum_df["profession"] = diffnum_df["base_string_direct"].apply(get_profession)
    diffnum_df["template"] = diffnum_df["base_string_direct"].apply(get_template)
    ...
    # diffnum_df["stereotypicality"] = woman_df["profession"].apply(get_stereotypicality)
    ...
    diffnum_df["neuron"] = (
        diffnum_df["layer_direct"].map(str) + "-" + diffnum_df["neuron_direct"].map(str)
    )
    neuron_effect_df = (
        diffnum_df.groupby("neuron")
        .agg(
            {
                "layer_direct": ["mean"],
                "neuron_direct": ["mean"],
